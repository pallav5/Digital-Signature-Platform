...
def sign_proposal(proposal_id):
    ...
    # ==============================================
    # CHECK FRAUD RISK BEFORE SIGNING (BACKEND)
    # ==============================================
    ...
    # Create signature record with device info
    ...
    # Regenerate PDF WITH signature and device info
    signed_pdf_bytes = generate_pdf(proposal, signature)
    with open(proposal.pdf_path, 'wb') as f:
...
def verify_signature(proposal_id):
    ...
    # Check if this is an old proposal without document_hash
    ...
    # Use the STORED document hash
    try:
        document_hash = bytes.fromhex(signature.document_hash)
